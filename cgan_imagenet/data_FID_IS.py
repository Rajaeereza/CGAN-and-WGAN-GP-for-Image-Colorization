...
def process_for_FID(data, device, fake_data = False):
  ...
  if fake_data:
    for j in range(len(data)):
      lab = torch.cat([data[j][0], data[j][1]], dim=1)
      lab_r = tr(lab)
      lab_norm = lab_r*0.5+0.5 #normalize between [0,1]
      lab_norm = torch.squeeze(lab_norm,0).permute(1,2,0).cpu()
      lab_arr = np.array(lab_norm)
      img_rgb_arr = lab2rgb(lab_arr).astype("float32") # Converting L*a*b to RGB
    ...
    for l, ab in data :
      ...
      lab_r = tr(lab)
      lab_norm = lab_r*0.5+0.5 #normalize between [0,1]
      lab_norm = lab_norm.permute(1,2,0)
      lab_arr = np.array(lab_norm)
      img_rgb_arr = lab2rgb(lab_arr).astype("float32") # Converting L*a*b to RGB
      img_rgb_tensor = transToTensor(img_rgb_arr)
      lab_tensor_batch = torch.unsqueeze(img_rgb_tensor,0)
      list_for_cat.append(lab_tensor_batch)
    image_for_FID = torch.cat(list_for_cat, dim=0).to(device)

  return image_for_FID

def data_FID(list_image_real, list_image_fake, device):

  valid_real = process_for_FID(list_image_real, device=device, fake_data = False)
  valid_fake = process_for_FID(list_image_fake, device=device, fake_data = True)
