...
##function for load and save the data pathes /// if saving = True, function use the saved files otherwise it will load and save them
def DataFromPath( PATH, number_of_data, saving = True, file_name = "train.npy"):  
  if saving:
...
def data(PATH, number_of_data, saving, batch_size):

  path_train = DataFromPath( PATH[0], number_of_data[0], saving[0], file_name = "train.npy")
  path_vt = DataFromPath( PATH[1], number_of_data [1], saving[1], file_name = "val_test.npy")
  
  path_test = path_vt[0:20]
  path_val = path_vt[20:]
  
  datset_tensor_train = Imagenetmini(path_train)
  datset_tensor_val = Imagenetmini(path_val, train_mode = False)
  datset_tensor_test = Imagenetmini(path_test, train_mode = False)
